...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const DEFAULT_DRES_SESSION_ID = '94WWzr8ZaEzYqhZNAOhWiuLuJ6cq9CG4'; // !!! THAY THẾ BẰNG SESSION ID THẬT CỦA BẠN
        ...
        switch (action) {
            ...
            case 'special_submission_alert':
                if (payload && payload.username) {
                    showGlobalSpecialSubmissionAlert(payload.username);
                }
                break;
        }
    }
    ...
    function showGlobalSpecialSubmissionAlert(username) {
        // Prevent creating multiple alerts at once
        if (document.querySelector('.global-alert-notification')) {
            return;
        }

        const alertDiv = document.createElement('div');
        alertDiv.className = 'global-alert-notification';
        alertDiv.innerHTML = `<span>${username.toUpperCase()}</span> ĐÚNG MẸ NÓ RỒI NỘP ĐI!`;
        ...
        // Set a timer to automatically hide the alert
        setTimeout(() => {
            alertDiv.classList.remove('show');
            // After the fade-out animation ends, remove the element from the DOM
            alertDiv.addEventListener('transitionend', () => {
                alertDiv.remove();
            });
        }, 800); // The alert will be visible for 3.5 seconds
    }
    ...
    function renderFullQueue(queueItems) {
        ...
        submitQueueFrames.forEach((frameData) => {
            ...
            frameElement.className = `queue-frame-item ${hasVotesClass} ${isSelectedClass} ${isFromVideoClass} ${isSpecialClass}`;
...
showToastNotification('Đã xóa lịch sử tìm kiếm!', 'success');
    ...
    /**
     * Xử lý khi nhấn nút Submit: Gửi dữ liệu lên backend
     */
    ...
    /**
     * Xóa sạch Form Submit Queue và reset trạng thái
     */
    ...
    /**
     * Xử lý việc tua video bằng con lăn chuột.
     */
    ...
    /**
     * Chụp frame hiện tại, kiểm tra ghi đè và gửi lên server.
     * @param {number} eventNumber - Số thứ tự sự kiện (1-5).
     */
