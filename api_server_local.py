...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const DEFAULT_DRES_SESSION_ID = 'pjWPrOkYKHAcPKx6Zvx5xeugd_TPC0e_'; // !!! THAY THẾ BẰNG SESSION ID THẬT CỦA BẠN
    ...
    function connectWebSocket() {
     ...
     function handleWebSocketMessage(message) {
        ...
        if (action === 'dres_submission_wrong') {
            ...
            showToastNotification(`Submission của ${payload.submittedBy} đã sai!`, 'error');
        ...
        switch (action) {
            ...
            case 'special_submission_alert':
                if (payload && payload.username) {
                    showGlobalAlert(payload.submittedBy, 'special');
                }
                break;
...
function showGlobalAlert(username, type = 'special') {
    ...
    } else { // Mặc định là 'special'
        config.text = `<span>${username.toUpperCase()}</span> ĐÚNG MẸ NÓ RỒI NỘP ĐI!`;
        config.cssClass = 'is-special'; // Class cho màu đỏ
    }
